...
$serverlar= array(
		array('id'=>'0','ad'=>'Tüm Sunucular',),
		array('id'=>'1','ad'=>'Mario Kart',),
		array('id'=>'2','ad'=>'#2',),
		array('id'=>'3','ad'=>'Trade',),
		array('id'=>'4','ad'=>'Pre. Trade',),
		array('id'=>'5','ad'=>'Orange X7',),
		array('id'=>'6','ad'=>'#2',),
		array('id'=>'7','ad'=>'Deathrun',),
		array('id'=>'8','ad'=>'Saxton Hale',),
		array('id'=>'9','ad'=>'Saxton #2',),
		array('id'=>'10','ad'=>'Jailbreak',),
		array('id'=>'11','ad'=>'#2',),
		array('id'=>'12','ad'=>' Surf ',),
		array('id'=>'13','ad'=>'Slender Fortress',),
		array('id'=>'14','ad'=>'Jump',),
		array('id'=>'15','ad'=>'Public',),
		array('id'=>'16','ad'=>'Medieval Wars',),
		array('id'=>'17','ad'=>'MvM',),
		array('id'=>'18','ad'=>'#2',)
	);
...
<!--
	<li><a href="?server=0" <?php if ($aktif == "0" || $aktif == "") { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(0) ?></a> </li>
    <li><a href="?server=1" <?php if ($aktif == "1") { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(1) ?></a> </li>
    <li><a href="?server=3" <?php if ($aktif == '3') { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(3) ?></a> </li>
    <li><a href="?server=5" <?php if ($aktif == '5') { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(5) ?></a> </li>
    <li><a href="?server=7" <?php if ($aktif == '7') { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(7) ?></a> </li>
    <li><a href="?server=8" <?php if ($aktif == '8') { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(8) ?></a> </li>
    <li><a href="?server=10" <?php if ($aktif == '10') { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(10) ?></a> </li>
    <li><a href="?server=12" <?php if ($aktif == '12') { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(12) ?></a> </li>
    <li><a href="?server=13" <?php if ($aktif == '13') { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(13) ?></a> </li>
    <li><a href="?server=14" <?php if ($aktif == '14') { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(14) ?></a> </li>
    <li><a href="?server=17" <?php if ($aktif == '17') { echo "class='logSunucularAktif'"; } ?>><?php echo aktifKim(17) ?></a> </li>
-->
 	<form class="logIcindeAra" name="form1" action="log/?server=<?php echo $aktif; ?>&ara=<?php echo $_POST["ara"]; ?>" method="post" id="logArama">
    	<input type="text" name="ara" placeholder="Kelime, SteamID vs." value="<?php if(!empty($_POST["ara"])) echo $_POST["ara"]; ?>">
...
<!--<div id="logFiltreleme">
	<form  method="get" action="test.php?server=<?php echo $aktif; ?>&sayfa=<?php echo $sayfa; ?>&sirala">
...
else { $query1 = "SELECT * FROM $tablo $sifirKontrol LIMIT $sayfaBasla, $limit"; }

$result = mysql_query($query1);

if(!empty($result)){ echo '<table id="logTablo" cellspacing="0">
	<tr>
    	<td>Tarih ve Saat</td>
    	<td>Sunucu</td>
        <td>SteamID</td>
...
while($yaz=mysql_fetch_array($result)){
		if($yaz["srvid"]==1) $sid = "Mario Kart";
		if($yaz["srvid"]==2) $sid = "Mario Kart #2";
		if($yaz["srvid"]==3) $sid = "Trade";
		if($yaz["srvid"]==4) $sid = "Premium Trade";
		if($yaz["srvid"]==5) $sid = "Orange X7";
		if($yaz["srvid"]==6) $sid = "Orange X7 #2";
		if($yaz["srvid"]==7) $sid = "Deathrun";
		if($yaz["srvid"]==8) $sid = "Saxton Hale";
		if($yaz["srvid"]==9) $sid = "Saxton Hale #2";
		if($yaz["srvid"]==10) $sid = "Jailbreak";
		if($yaz["srvid"]==11) $sid = "Jailbreak #2";
		if($yaz["srvid"]==12) $sid = "Surf";
		if($yaz["srvid"]==13) $sid = "Slender Fortress";
		if($yaz["srvid"]==14) $sid = "Jump";
		if($yaz["srvid"]==15) $sid = "Public";
		if($yaz["srvid"]==16) $sid = "Medieval Wars";
		if($yaz["srvid"]==17) $sid = "MvM #1";
		if($yaz["srvid"]==18) $sid = "MvM #2";
		if($yaz["team"]==1)
		{
